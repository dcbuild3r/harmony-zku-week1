...
template sudoku() {
    ...
    // we have 9 numbers per each row and per each column, and we need to check the puzzle against the solution
    component rangeTensor[9][9][2];
    

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {

            // biggest number represented is 9 -> fits into 4 bits ... 9 < 16
            rangeTensor[i][j][0] = RangeProof(4);
            rangeTensor[i][j][1] = RangeProof(4);

            // puzzle assertions
            rangeTensor[i][j][0].range[0] <== 0;
            rangeTensor[i][j][0].range[1] <== 9;
            rangeTensor[i][j][0].in <== puzzle[i][j];
            // double check conditions are met
            rangeTensor[i][j][0].out === 1;

            // solution assertions
            rangeTensor[i][j][1].range[0] <== 0;
            rangeTensor[i][j][1].range[1] <== 9;
            rangeTensor[i][j][1].in <== solution[i][j];
            // double check conditions are met
            rangeTensor[i][j][1].out === 1;
